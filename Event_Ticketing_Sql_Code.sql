...
END $$
DELIMITER ;
INSERT INTO Reservations (reservation_id, customer_id, event_id, ticket_id, reservation_date)
VALUES (6666, 1, 1000, 103, '2025-10-09');
SELECT * FROM Tickets WHERE ticket_id = 103;
...
BEGIN
    ...
    FROM Sales s
    JOIN Reservations r ON s.reservation_id = r.reservation_id6dh77e63kb vg  uj              m
    JOIN Events e ON r.event_id = e.event_id;
END $$
DELIMITER ;
CALL GetEvent();
...
-- Q11. Window Functions - Find the highest revenue-generating event and rank all events by total revenue
SELECT 
    e.event_name,
    SUM(s.amount) AS total_revenue,
...
-- Q12. DATE Functions - Find the number of days left for each upcoming event.
SELECT 
    event_name,
    event_date,
...
-- Q13.CASE Statements (Conditional Logic in Queries) - Categorize ticket prices as ‘Premium’, ‘Standard’, or ‘Budget’.
SELECT 
    t.ticket_id,
    e.event_name,
    t.price,
